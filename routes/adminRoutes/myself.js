const router = require("express").Router();
const { upload, handleImageUpload } = require("../../config/multer-config");
const adminController = require("../../controllers/adminController/adminController");
const authMiddleware = require("../../middlewares/auth");
const userValidation = require("../../middlewares/userValidation");

router.put("/update", upload.single("user_picture"), authMiddleware, userValidation.updateUserValidation, handleImageUpload("users"), adminController.updateUser);
router.post("/login", userValidation.loginValidation, adminController.login);
router.post("/register", upload.single("user_picture"), handleImageUpload("users"), adminController.register);
router.get("/get-infos", authMiddleware, adminController.getInfos);
router.post("/verify-password", authMiddleware, adminController.verifyPassword);

module.exports = router;
